import { useState } from "react";
import person from "./assets/person-unsplash.jpg";
import googleIcon from "./assets/googlelogo_color_272x92dp.png";

function App() {
  const [typedText, setTypedText] = useState("");

  const [suggestions, setSuggestions] = useState(Array(7).fill(""));

  const generateArray = (value) => {
    return Array(8)
      .fill(value)
      .map((val, index) => `${val} - suggestion  ${index + 1}`);
  };

  const handleChange = (value) => {
    setTypedText(value);

    const newSuggestions = generateArray(value);
    setSuggestions(newSuggestions);
  };

  const closeSuggestions = () => {
    setTypedText("");
  };

  return (
    <div className="h-screen w-screen relative p-0 m-0 overflow-hidden">
      <div className="flex w-full justify-end items-center px-4 py-3 gap-6 absolute top-0 left-0">
        <a href="" className="text-sm">
          Gmail
        </a>
        <a href="" className="text-sm">
          Images
        </a>

        <svg
          focusable="false"
          viewBox="0 0 24 24"
          class="h-6 w-6 fill-gray-600"
        >
          <path d="M6,8c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2zM12,20c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2zM6,20c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2zM6,14c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2zM12,14c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2zM16,6c0,1.1 0.9,2 2,2s2,-0.9 2,-2 -0.9,-2 -2,-2 -2,0.9 -2,2zM12,8c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2zM18,14c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2zM18,20c1.1,0 2,-0.9 2,-2s-0.9,-2 -2,-2 -2,0.9 -2,2 0.9,2 2,2z"></path>
        </svg>

        <img
          src={person}
          className="img h-8 w-8 object-cover rounded-full"
        ></img>
      </div>

      {/* Main content */}
      <main className="h-full w-full flex flex-col items-center mt-32">
        {/* Search Bar and Search Suggestions container */}
        <div className="w-full grid gap-5">
          <div className="w-full m-auto flex items-center justify-center">
            <img src={googleIcon} alt="googleIcon" className="md:h-24 h-12" />
          </div>

          {/* Search bar */}
          <div
            className={`lg:w-[580px] md:w-10/12 w-11/12 m-auto border-[1px] border-gray-200 relative ${
              typedText ? "rounded-3xl" : "rounded-full"
            } hover:shadow-[0_1px_6px_1px_rgba(32,33,36,0.28)]`}
          >
            <form className="h-12 w-full flex justify-between items-center gap-4 px-3">
              {/* Search icon */}
              <svg
                focusable="false"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                className="h-7 w-7 fill-primary"
              >
                <path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-9 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path>
              </svg>

              {/* Search input */}
              <input
                type="text"
                className="w-full h-full outline-none"
                value={typedText}
                onChange={(e) => handleChange(e.target.value)}
              />

              {/* Close button */}
              {typedText && (
                <div
                  className="px-2.5 border-r-[1px] border-gray-400"
                  onClick={() => {
                    setTypedText("");
                  }}
                >
                  <svg
                    className="h-6 w-6 cursor-pointer fill-gray-500"
                    focusable="false"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                  >
                    <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"></path>
                  </svg>
                </div>
              )}

              {/* Mic icon */}
              <svg
                className="h-9 w-9 cursor-pointer"
                focusable="false"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fill="#4285f4"
                  d="m12 15c1.66 0 3-1.31 3-2.97v-7.02c0-1.66-1.34-3.01-3-3.01s-3 1.34-3 3.01v7.02c0 1.66 1.34 2.97 3 2.97z"
                ></path>
                <path fill="#34a853" d="m11 18.08h2v3.92h-2z"></path>
                <path
                  fill="#fbbc04"
                  d="m7.05 16.87c-1.27-1.33-2.05-2.83-2.05-4.87h2c0 1.45 0.56 2.42 1.47 3.38v0.32l-1.15 1.18z"
                ></path>
                <path
                  fill="#ea4335"
                  d="m12 16.93a4.97 5.25 0 0 1 -3.54 -1.55l-1.41 1.49c1.26 1.34 3.02 2.13 4.95 2.13 3.87 0 6.99-2.92 6.99-7h-1.99c0 2.92-2.24 4.93-5 4.93z"
                ></path>
              </svg>

              {/* Google lens icon */}
              <svg
                className="h-9 w-9 cursor-pointer"
                alt="Camera search"
                focusable="false"
                viewBox="0 0 192 192"
                xmlns="http://www.w3.org/2000/svg"
              >
                <rect fill="none" height="192" width="192"></rect>
                <g>
                  <circle fill="#4285f4" cx="96" cy="104.15" r="28"></circle>
                  <path
                    fill="#ea4335"
                    d="M160,72v40.15V136c0,1.69-0.34,3.29-0.82,4.82v0v0c-1.57,4.92-5.43,8.78-10.35,10.35h0v0 c-1.53,0.49-3.13,0.82-4.82,0.82H66l16,16h50h12c4.42,0,8.63-0.9,12.46-2.51c3.83-1.62,7.28-3.96,10.17-6.86 c1.45-1.45,2.76-3.03,3.91-4.74c2.3-3.4,3.96-7.28,4.81-11.44c0.43-2.08,0.65-4.24,0.65-6.45v-12V96.15V84l-6-19l-10.82,2.18 C159.66,68.71,160,70.31,160,72z"
                  ></path>
                  <path
                    fill="#4285f4"
                    d="M32,72c0-1.69,0.34-3.29,0.82-4.82c1.57-4.92,5.43-8.78,10.35-10.35C44.71,56.34,46.31,56,48,56 h96c1.69,0,3.29,0.34,4.82,0.82c0,0,0,0,0,0L149,45l-17-5l-16-16h-13.44H96h-6.56H76L60,40H48c-17.67,0-32,14.33-32,32v12v20l16,16 V72z"
                  ></path>
                  <path
                    fill="#34a853"
                    d="M144,40h-12l16.83,16.83c1.23,0.39,2.39,0.93,3.47,1.59c2.16,1.32,3.97,3.13,5.29,5.29 c0.66,1.08,1.2,2.24,1.59,3.47v0L176,84V72C176,54.33,161.67,40,144,40z"
                  ></path>
                  <path
                    fill="#fbbc04"
                    d="M48,168h39.89l-16-16H48c-8.82,0-16-7.18-16-16v-23.89l-16-16V136C16,153.67,30.33,168,48,168z"
                  ></path>
                </g>
              </svg>
            </form>

            {typedText && (
              <div className="bg-white w-full z-50 pb-5 rounded-3xl">
                {/* Search Suggestions */}
                <div className="flex gap-1 flex-col max-h-64 w-full">
                  {suggestions.map((suggestion) => (
                    <div
                      className="flex gap-6 w-full px-3 py-1 hover:bg-gray-100/70 cursor-pointer"
                      onClick={() => {
                        setTypedText(`Your selected search - "${suggestion}"`);
                      }}
                    >
                      {/* Search icon */}
                      <svg
                        focusable="false"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        className="h-5 w-5 fill-primary"
                      >
                        <path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-9 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path>
                      </svg>

                      <p className="w-full overflow-hidden text-ellipsis whitespace-nowrap">
                        {suggestion}
                      </p>
                    </div>
                  ))}
                </div>

                {/* Google Search button */}
                <div className="w-full flex gap-4 justify-center items-center mt-10">
                  <button
                    onClick={closeSuggestions}
                    className="px-5 py-2 bg-gray-100 text-sm rounded-sm focus:border-blue-700 hover:shadow-sm hover:border-[1px] border-gray-200"
                  >
                    Google Search
                  </button>

                  <button
                    onClick={closeSuggestions}
                    className="px-5 py-2 bg-gray-100 text-sm rounded-sm focus:border-blue-700 hover:shadow-sm hover:border-[1px] border-gray-200"
                  >
                    I'm Feeling Lucky
                  </button>
                </div>
              </div>
            )}
          </div>

          {/* Other google buttons */}
          {!typedText && (
            <div className="-mt-5">
              <div className="w-full flex gap-4 justify-center items-center mt-10">
                <button className="px-5 py-2 bg-gray-100 text-sm rounded-sm focus:border-blue-700 hover:shadow-sm hover:border-[1px] border-gray-200">
                  Google Search
                </button>

                <button className="px-5 py-2 bg-gray-100 text-sm rounded-sm focus:border-blue-700 hover:shadow-sm hover:border-[1px] border-gray-200">
                  I'm Feeling Lucky
                </button>
              </div>

              <div className="w-full flex gap-3 justify-center items-center mt-6 flex-wrap">
                <span className="text-sm">Google offered in:</span>

                <a className="text-sm text-[#1a0dab] hover:underline cursor-pointer">
                  Hausa
                </a>

                <a className="text-sm text-[#1a0dab] hover:underline cursor-pointer">
                  Igbo
                </a>

                <a className="text-sm text-[#1a0dab] hover:underline cursor-pointer">
                  Èdè Yorùbá
                </a>

                <a className="text-sm text-[#1a0dab] hover:underline cursor-pointer">
                  Nigerian Pidgin
                </a>
              </div>
            </div>
          )}
        </div>
      </main>

      {/* Footer stuff */}
      <footer className="absolute bottom-0 left-0 w-full bg-[#F2F2F2] -z-10">
        <div className="w-full md:py-3.5 py-1 md:px-7 px-2 text-gray-600 border-b-[1px] border-b-gray-300">
          <p className="text-semibold">Nigeria</p>
        </div>

        <div className="md:py-3.5 py-1 md:px-7 px-2 flex md:flex-row flex-col md:justify-between md:items-center relative z-20 md:space-y-0 space-y-2">
          {/* Left side links */}
          <div className="flex gap-4 text-gray-600 md:text-sm text-xs">
            <p className="text-semibold cursor-pointer">About</p>

            <p className="text-semibold cursor-pointer">Advertising</p>

            <p className="text-semibold cursor-pointer">Business</p>

            <p className="text-semibold cursor-pointer">How Search works</p>
          </div>

          {/* Middle side links */}
          <div className="flex-gap-2 items-center-justify-center text-gray-600 md:text-sm text-xs md:absolute text-center md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2">
            <p className="text-semibold">🍀 Carbon neutral since 2007</p>
          </div>

          <div className="flex gap-4 text-gray-600 md:text-sm text-xs">
            <p className="text-semibold cursor-pointer">Privacy</p>

            <p className="text-semibold cursor-pointer">Terms</p>

            <p className="text-semibold cursor-pointer">Setting</p>
          </div>
        </div>
      </footer>
    </div>
  );
}

export default App;
